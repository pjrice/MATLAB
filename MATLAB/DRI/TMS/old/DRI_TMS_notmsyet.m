%DRI paradigm with EMG recordings and TMS stimluation
...
% numblocks = input('How many blocks to work? ');
...
%Setup communication with Magstim
...
%I believe the above lines maintain the communication - this next one just
%causes it to wait for 150ms?
Rapid2_Delay(150, serialPortObj);

% Set initial power level to 30% of output
powerLevel = input('Set TMS intensity (% of output): ');
...
%Right now let's just arm the system at the very beginning as well as 
%disabling the coil safety switch (cause who needs that shit?) and disarm 
%it at the very end - might have to change this to arming/disarming every 
%time we want to deliver stimulation
...
dblUDBacklog = 0;
scanRate = 3000;
delay = 10; %in seconds - this is the maximum possible trial length
...
%Give the driver a 15 second buffer (scanRate * 2 channels * 15 seconds).
%NOTE: REFINE BUFFER DURATION
ljudObj.AddRequest(ljhandle, LabJack.LabJackUD.IO.PUT_CONFIG, LabJack.LabJackUD.CHANNEL.STREAM_BUFFER_SIZE, scanRate*2*15, 0, 0);
...
%right now, predetermined PMd and PFC stim/sham blocks; if we want to
%randomize, experimenter will have to know in order to change
%sham/stim coils/coil location
%0==PMd; 1==PFC
%right now alternate stim/sham (so odd numbered blocks are stimulation,
%even numbered blocks are sham) - up to experimenter to change to right
%coil between blocks

PMd_PFC = [0 0 1 1 0 0 1 1];
...
%should preallocate these at some point
...
max_tlength = 10;  %seconds; CHANGE VALUE 'DELAY' ABOVE FOR EMG
...
for block = 1:numblocks
    ...
    for trial = 1:numtrials
        ...
        if rpspchooser(trial,block)==0  %TMS before rule presentation
            ...
            %very basic right now; triggers one pulse between fixation
            %period and RP. Eventually want rTMS during FP or RP 
        ...
        else  %TMS before stimulus presentation
            ...
            %trigger the stimulator
            %very basic right now; triggers one pulse between fixation
            %period and SP. Eventually want rTMS during FP or SP 
            success = Rapid2_TriggerPulse(serialPortObj, 1);
            if ~success
                display 'Error: Cannot trigger a pulse';
                return
...
%get rid of some stupid shit that'll fuck with saving the file
clear e eNet ljasm ljerror ljudObj serialPortObj
