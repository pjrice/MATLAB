...
% This is an n row array
%the first row will record symbol(0) or finger(1) rule conditions
%2nd: the brain structure we are "in"
%3rd: whether stimulation occurred or not - right now pointless
%4th: the number that was presented
%5th: unused
%6th: the key (0 for spatially left key, 1 for right key, irregardless of 
    %hand) they actually responded with -assuming left hand
%7th: the finger they were supposed to respond with
%8th: the finger they actually responded with
%9th: the symbol that was assigned to even
%10th: the symbol that was on the left of the stimulus screen
%11th: did they get it right? (0==no;1==yes)
%12th: the interstimulus delay (in sec)
...
for trial = 1:numtrials
    ...
    respMat{12,trial} = isdTimeSecs(trial,1);
   ...
   %present rule
    if condMatrixShuffled(1,trial) == 0   %0==ID 1==NID
        ...
        printsymbols_l2 = ['\n\n ODD -> ' symbolchooser(2)];
        respMat{9,trial} = symbolchooser(1);
        ...
        %record which finger they were supposed to respond with
        if mod(stim,2)
            respMat{7,trial} = fingers{fingerchooser(2)};
        else
            respMat{7,trial} = fingers{fingerchooser(1)};
        end
...
%present stimulus, record key pressed and RT
    ...
    respMat{10,trial} = symbols(fingerchooser(1));
    if mod(stim,2) && strcmp('A',symbolchooser(2))
