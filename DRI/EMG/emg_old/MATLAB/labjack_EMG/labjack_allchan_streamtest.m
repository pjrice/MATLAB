%test of streaming all channels of LabJAck connector
...
scanRate = 3000;
delay = 1; %in seconds
numScans = 6000;  %2x the expected # of scans (2*scanRate*delayms/1000)
numScansRequested = 0;
...
try
    ...
    %Configure all flexible channels as analog.  That means we
    %will start from channel 0 and update all 16 flexible bits.  We will
    %pass a value of b1111111111111111.
    ...
    %function overload.
    ljudObj.ePut(ljhandle, LabJack.LabJackUD.IO.PUT_ANALOG_ENABLE_PORT, 0, 65525, int32(16));
    ...
    ljudObj.AddRequest(ljhandle, LabJack.LabJackUD.IO.PUT_CONFIG, LabJack.LabJackUD.CHANNEL.STREAM_BUFFER_SIZE, scanRate*4*5, 0, 0);
...
%     ljudObj.AddRequest(ljhandle, LabJack.LabJackUD.IO.ADD_STREAM_CHANNEL, 12, 0, 0, 0);
%     ljudObj.AddRequest(ljhandle, LabJack.LabJackUD.IO.ADD_STREAM_CHANNEL, 13, 0, 0, 0);
%     ljudObj.AddRequest(ljhandle, LabJack.LabJackUD.IO.ADD_STREAM_CHANNEL, 14, 0, 0, 0);
%     ljudObj.AddRequest(ljhandle, LabJack.LabJackUD.IO.ADD_STREAM_CHANNEL, 15, 0, 0, 0);
    ...
    disp(['Actual Sample Rate = ' num2str(16*dblValue) sprintf('\n')]) % # channels * scan rate
    ...
    for i=1:loopAmount
        ...
        adblData = NET.createArray('System.Double', 4*numScans);  %Max buffer size (#channels*numScansRequested)
